...
void setup() {
  ...
  Serial.println("--------------------------------------------");
  Serial.println("Arduino nano 33 ble sense Sense running TensorFlow Lite Micro");
  Serial.println("");
  pinMode(LEDR,OUTPUT);
  pinMode(LEDG,OUTPUT);
  pinMode(LEDB,OUTPUT);
...
void loop() {
  ...
  if (p == 0 && c <= 30 && sum > 0) {
    ...
    if(int(tflOutputTensor->data.f[1] * 100)>=90){
    ...
    if(int(tflOutputTensor->data.f[2] * 100)>=90){
      digitalWrite(LEDR,HIGH);
